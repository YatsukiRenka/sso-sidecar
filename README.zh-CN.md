...
> 本文档是英文 README 的翻译；两者不一致时以英文版为准。

一个部署在 [Authentik](https://goauthentik.io/) 代理 outpost 与
[SillyTavern](https://github.com/SillyTavern/SillyTavern) 之间的 fail-closed
反向代理。它将稳定的 Authentik UID 映射到 SillyTavern handle，安全地预配账户、
同步管理员组成员身份，并为兼容 OpenAI 的 LLM API 提供范围严格受限、使用服务
身份验证的中继。

![Python 3.12](https://img.shields.io/badge/python-3.12-blue)
![依赖 aiohttp](https://img.shields.io/badge/deps-aiohttp-green)
[![许可证 AGPL-3.0](https://img.shields.io/badge/license-AGPL--3.0-blue)](LICENSE)
[![测试](https://github.com/YatsukiRenka/sso-sidecar/actions/workflows/test.yml/badge.svg)](https://github.com/YatsukiRenka/sso-sidecar/actions/workflows/test.yml)

## 目录

- [功能概览](#功能概览)
- [安全模型](#安全模型)
- [SillyTavern 必需配置](#sillytavern-必需配置)
- [配置](#配置)
  - [SSO 与账户预配](#sso-与账户预配)
  - [LLM 中继](#llm-中继)
- [Compose 示例](#compose-示例)
  - [Linux 宿主机权限](#linux-宿主机权限)
- [更新](#更新)
- [映射与故障行为](#映射与故障行为)
- [开发](#开发)
- [许可证](#许可证)
...
- **为 SillyTavern 提供单点登录**：Authentik 是身份与角色的唯一来源；
  SillyTavern 自带的密码登录、找回、本地用户管理端点一律不转发。
- **稳定身份**：一旦记录了某个 UID，即使 Authentik username 发生变化，它也
  始终映射到同一个 handle。
- **安全预配**：通过 SillyTavern 管理员 API 创建缺失账户，密码由确定性算法
  派生且熵值高，绝不会返回给浏览器；管理员权限按 Authentik 组对账。
- **范围受限的 LLM 中继**：只有三个路由的 `/v1` 端点让受管用户可以访问上游
  兼容 OpenAI 的 API，而无需持有真实密钥。
- **全链路 fail-closed**：来源地址不受信任、身份请求头缺失、预配或存储失败
  时一律拒绝，不做降级放行。

**运行前提**：一个 Authentik 代理 outpost、一个部署在私有网络中的
SillyTavern 实例，以及 Docker（见 [Compose 示例](#compose-示例)）或
Python 3.12 + `aiohttp`。

初次部署？建议先读[安全模型](#安全模型)，按 [SillyTavern
必需配置](#sillytavern-必需配置)改好设置，再用 [Compose
示例](#compose-示例)部署。
