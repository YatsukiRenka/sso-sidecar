...
一个部署在 Authentik 代理 outpost 与 SillyTavern 之间、失败时拒绝
（fail-closed）的反向代理。它将稳定的 Authentik UID 映射到 SillyTavern
账户名，安全地预配账户、同步管理员组成员身份，并为兼容 OpenAI 的 LLM API
提供范围严格受限、使用服务身份验证的中继。
...
`ALLOW_USERNAME_LINKING=false` 可防止他人通过复用用户名接管已有的
...
- 状态文件中已有的 UID 始终映射到同一个账户名，即使 Authentik 用户名发生
  变化，或新用户名无法转换为 ASCII 账户名也是如此。
- 一个 UID 不能重新绑定到另一个账户名，一个账户名也不能绑定到多个 UID。
